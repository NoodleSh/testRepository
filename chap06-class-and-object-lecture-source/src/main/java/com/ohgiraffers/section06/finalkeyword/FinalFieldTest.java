...
public class FinalFieldTest {
    ...
    * 5. non-static 메소드 : 메소드 재작성(overrridng) 불가
    * 6. static 메소드 : 메소드 재작성(overridng) 불가
    * 7. 클래스 : 상속 불가
    * */
 ...
 //   private final int STATIC_NUM; // 0으로 초기화되어 이후 변경 불가능하기 때문에 에러남
    ...
    /*
    * 생성자를 이용한 초기화는 불가능하다.
    * 생성자는 인스턴스(객체)가 생성되는 시점에 호출이 되기 떄문에 그 전에는 초기화가 일어나지 못한다.
    * 하지만 static은 프로그램이 start될 때 할당되기 떄문에 초기화 되지 않은 상태로 선언된 것과 동일하여
    * 기본값으로 초기화 된 후 값을 변경하지 못하기 때문에 에러가 발생한다.(final 키워드를 썼을때)
    * */
